...
if(isset($_POST["action"])) //Check value of $_POST["action"] variable value is set to not
{
 ...
 {
  $statement = $connection->prepare("SELECT * FROM yillikbakim ORDER BY id DESC LIMIT 500");
  $statement->execute();
...
if (!empty($pdf_paths)) {
    ...
    $query = "INSERT INTO yillikbakim (ekipmanadi, bakimdetay, bakimperiyodu, sorumlutedarikci, plantarihi, gerceklesentarih, pdf) 
              VALUES (:ekipmanadi, :bakimdetay, :bakimperiyodu, :sorumlutedarikci, :plantarihi, :gerceklesentarih, :pdf)";

    $statement = $connection->prepare($query);

    $result = $statement->execute(
        array(
            ':ekipmanadi'             => $_POST["ekipmanadi"],
            ':bakimdetay'            => $_POST["bakimdetay"],
            ':bakimperiyodu'             => $_POST["bakimperiyodu"],
            ':sorumlutedarikci'             => $_POST["sorumlutedarikci"],
            ':plantarihi'             => $_POST["plantarihi"],
            ':gerceklesentarih'             => $_POST["gerceklesentarih"],
            ':pdf'              => $pdf_paths_str, 
        )
    );
 ...
 if($_POST["action"] == "Select" )
{
    ...
    {
        $output["ekipmanadi"]        = $row["ekipmanadi"];
        $output["bakimdetay"]       = $row["bakimdetay"];
        $output["bakimperiyodu"]        = $row["bakimperiyodu"];
        $output["sorumlutedarikci"]        = $row["sorumlutedarikci"];
        $output["plantarihi"]        = $row["plantarihi"];
        $output["gerceklesentarih"]        = $row["gerceklesentarih"];
        $output["pdf"]  = $pdf_path;
...
if($_POST["action"] == "Güncelle") {
    ...
    $statement = $connection->prepare("UPDATE yillikbakim 
        SET ekipmanadi = :ekipmanadi, bakimdetay = :bakimdetay, bakimperiyodu = :bakimperiyodu, sorumlutedarikci = :sorumlutedarikci, plantarihi = :plantarihi, gerceklesentarih = :gerceklesentarih, pdf = :pdf WHERE id = :id");
