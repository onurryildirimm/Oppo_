...
$statement = $pdoconnection->prepare("SELECT * FROM ozelisler ORDER BY id DESC LIMIT 1500");
$statement->execute();
...
if ($statement->rowCount() > 0) {
    ...
    $output .= '
    <tr>
        <td align="center" colspan="8">Veri Bulunamadı.</td>
    </tr>';
}
$output .= '</table>';
echo $output;
 ...
 $('#action').click(function(){
  ...
  if(tarih != '' && konu != '') //This condition will check both variable has some value
  {
   $.ajax({
    ...
    data:{tarih:tarih, konu:konu, detay:detay, durum:durum, sorumludepartman:sorumludepartman, yapilanis:yapilanis, bitistarihi:bitistarihi, kullanici:kullanici, employee_id:id, action:action}, //Send data to server
