...
<!-- ============================================================== -->
   ...
   $statement = $pdoconnection->prepare("SELECT * FROM isler WHERE durumu = 'TAMAMLANDI' ORDER BY id DESC LIMIT 500");
  $statement->execute();
  ...
  if($statement->rowCount() > 0)
  {
   foreach($result as $row)
   {
    ...
    <tr>
     ...
     <td>'.$row["durumu"].'</td>
     <td>'.date('d.m.Y', strtotime($row["kapanistarihi"])).'</td>
